...
function startDiceHangWatchdog(diceEnabled) {
  ...
  const timerId = setTimeout(async () => {
    if (!active) return;
    if (!window.isFaucetPage() || window.isDicebetPage() || window.isWithdrawPage()) return;

    const pluginTab = await window.isPluginTab();
    if (!pluginTab) return;

    const diceUrl = window.getDicePageUrl();
    window.log(`Faucet flow exceeded ${(window.FAUCET_HANG_TO_DICE_TIMEOUT_MS / 1000).toFixed(0)}s — forcing DiceBet load: ${diceUrl}`);
    window.sendPhaseHeartbeat("faucet-hang-timeout-to-dice");
...
function bonusExhausted() {
  ...
  if (badge && parseInt(badge.textContent) <= 0) {
    window.log("Bonus exhausted: free_spins badge = 0");
    return true;
  }
  ...
  for (const el of msgEls) {
    ...
    if (NO_MORE_PATTERNS.test(el.textContent)) {
      window.log(`Bonus exhausted: found message "${el.textContent.trim().substring(0, 50)}"`);
      return true;
    }
  }
  return false;
...
function findBonusTab() {
  ...
  if (el) {
    window.log(`Found bonus tab with selector`);
    return el;
  }
  ...
  for (const el of allElements) {
    ...
    if ((text === 'bonus' || text === 'bonus roll' || text === 'bonus faucet') && el.tagName !== 'BODY' && el.tagName !== 'HTML') {
      if (el.onclick || el.getAttribute('data-tab') || el.className.includes('tab') || el.className.includes('nav')) {
        window.log(`Found bonus tab by text search (exact): "${el.textContent.trim()}"`);
        return el;
      }
    }
    if (/^bonus|bonus roll|bonus faucet|bonus spins|free spins/.test(text) && el.className.includes('tab')) {
      window.log(`Found bonus tab by text pattern: "${el.textContent.trim()}"`);
      return el;
    }
  }
  return null;
...
function findClaimButtonInContext(context = document) {
  for (const el of __FP_Selectors.getAllValid('faucetClaimBtnContext', context)) {
    ...
    if (text.includes('dice') || onclick.includes('dice')) {
      window.log(`Skipping dice button: "${text.substring(0, 30)}"`);
      continue;
    }
    
    const trimText = (el.textContent || el.value || "").trim().toLowerCase();
    if (CLAIM_KEYWORDS.some(k => trimText.includes(k))) {
      window.log(`Found claim button in context: tag=${el.tagName}, text="${trimText.substring(0, 30)}"`);
      return el;
    }
  }
  ...
  if (btnByAttr) {
    ...
    if (!onclick.includes('dice')) {
      window.log(`Found claim button in context by attribute/class`);
      return btnByAttr;
    }
  }
  return null;
...
async function claimBonusFaucets() {
  try {
    ...
    if (!bonusTab) {
      window.log("No bonus faucet tab found — skipping bonus round");
      return;
    }
    ...
    if (!isNaN(spins) && spins <= 0) {
      window.log("Bonus faucet: 0 spins remaining — skipping");
      return;
    }

    window.log(`Clicking bonus tab: "${bonusTab.textContent.trim()}"`);
    bonusTab.click();
    ...
    window.log(`Bonus content container identified`);

    let consecutiveNoButton = 0;
    for (let round = 1; round <= 30; round++) {
      await window.sleep(round === 1 ? 1000 : 2500);

      if (bonusExhausted()) {
        window.log(`Bonus round ${round}: exhausted (badge check) — done`);
        break;
      }
      ...
      if (window.hasCaptchaWidget()) {
        window.log(`Bonus round ${round}: waiting for captcha…`);
        chrome.runtime.sendMessage({ type: "focus-tab" });
        await window.sleep(1000);
        
        let captchaResolved = false;
        for (let attempt = 0; attempt < 2; attempt++) {
          setTimeout(window.tryClickCaptchaWidget, 1500);
          const token = await Promise.race([
            window.waitForCaptchaToken(20000), 
            window.sleep(25000).then(() => null)
          ]);
          if (token) {
            window.log(`Bonus captcha resolved on attempt ${attempt + 1}`);
            captchaResolved = true;
            break;
          }
          window.log(`Bonus captcha timeout on attempt ${attempt + 1}, rotating…`);
          const rotated = await window.rotateCaptchaType();
          if (!rotated) break;
        }

        if (!captchaResolved) { 
          window.log("Bonus captcha failed all attempts — stopping bonus loop"); 
          break; 
        }
      ...
      if (!claimBtn) {
        consecutiveNoButton++;
        window.log(`Bonus round ${round}: no claim button found in bonus content (${consecutiveNoButton}x)`);
        if (consecutiveNoButton >= 2) {
          window.log("No button found twice in a row — stopping bonus loop");
          break;
        }
        continue;
      }
      ...
      window.log(`Bonus round ${round}: clicking claim/roll button "${claimBtn.textContent?.trim() || claimBtn.value}"`);
      claimBtn.click();
      await window.sleep(window.POST_CLAIM_WAIT_MS + 1000); 

      if (bonusExhausted()) {
        window.log(`Bonus round ${round}: exhausted after claim — done`);
        break;
      }
    }
    window.log("Bonus faucets claim loop completed");
  } catch (err) {
    window.log("ERROR in claimBonusFaucets:", err.message);
  }
}
...
function detectCooldown() {
  ...
  window.log(`Detected timer text: "${text}"`);
...
async function tryClaimHourlyFaucet() {
  ...
  if (!btn) {
    window.log("No hourly claim button found with keywords — trying fallback");
    btn = document.querySelector('button[type="submit"], input[type="submit"]');
  }

  if (btn) {
    if (window.hasLoginForm()) {
      window.log("ERROR: Login form detected. Aborting hourly claim.");
      return false;
    }
    window.log("Clicking hourly claim button:", btn.textContent?.trim() || btn.value);
    btn.click();
    await window.sleep(2000); // Give it a moment to start the request
    return true;
  }

  window.log("Hourly faucet seems to be on cooldown or button is missing.");
  return false;
}

window.runFaucet = async function runFaucet() {
  window.log("Faucet page:", location.href);

  await window.waitForRocketLoaderHandlers();
  const dbConfig = await window.getDicebetConfig();
  ...
  try {
    ...
    if (initialWait !== null) {
      if (initialWait > 2.5) {
        window.log(`Faucet is on long cooldown: ${initialWait} min. Reporting and aborting.`);
        chrome.runtime.sendMessage({ type: "faucet-cooldown", waitMinutes: initialWait });
        return;
      } else {
        window.log(`Faucet cooldown is short (${initialWait} min). Staying on page to prepare early.`);
      }
    }
    ...
    if (window.hasCaptchaWidget()) {
      ...
      while (captchaAttempts < maxCaptchaAttempts) {
        window.log(`Captcha attempt ${captchaAttempts + 1}/${maxCaptchaAttempts}...`);
        chrome.runtime.sendMessage({ type: "focus-tab" });
        await window.sleep(2500);
        window.tryClickCaptchaWidget();
        const token = await window.waitForCaptchaToken(40000);
        if (token) {
          window.log("✓ Captcha resolved");
          captchaResolved = true;
          break;
        }
        window.log(`✗ Captcha timeout on attempt ${captchaAttempts + 1}`);
        const rotated = await window.rotateCaptchaType();
        if (!rotated) break;
        captchaAttempts++;
      }
    } else {
      window.log("No captcha detected — proceeding.");
      captchaResolved = true;
    }
    ...
    if (initialWait !== null && initialWait <= 2.5) {
      ...
      window.log(`Waiting for exact expiration at system time... (${(waitMs/1000).toFixed(1)}s total)`);
      while (true) {
        ...
        if (remaining > 10000) {
          ...
          await window.sleep(Math.max(10, remaining));
          break;
        }
      }
      window.log("Exact cooldown has been reached! Claiming...");
    }

    // 2. Attempt Hourly Faucet
    const hourlyClaimed = await tryClaimHourlyFaucet();
    if (hourlyClaimed) {
      window.log("Hourly faucet claim submitted. Waiting...");
      await window.sleep(window.POST_CLAIM_WAIT_MS);
    }

    // 3. Attempt Bonus Faucets (always check)
    await claimBonusFaucets();

    const balance = window.readBalance();
    window.log("Final balance check:", balance);

    if (dbConfig.enabled && balance != null && balance > 0) {
      const diceUrl = window.getDicePageUrl();
      window.log(`DiceBet enabled, navigating to dice page: ${diceUrl}`);
      window.location.href = diceUrl;
      return;
    }

    window.log(`Faucet cycle completed.`);

    window.sendDone(balance);
  } finally {
